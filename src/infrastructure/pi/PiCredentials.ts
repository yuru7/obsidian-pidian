...
- get_note_metadata reads a Markdown note's frontmatter, tags, aliases, headings, embeds, list items, sections, outgoing links, and backlinks without reading the body. Optional fields limits which keys are returned. Line numbers are 1-based. Use this instead of read_note when you only need structure or links.
- get_vault_links reads vault-wide resolved and unresolved link maps. Optional path limits sources to that file or folder prefix. Optional fields selects resolvedLinks and/or unresolvedLinks. Optional limit caps how many source files are returned.
